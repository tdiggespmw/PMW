...
#Region "DataAccess"
    ...
    Public Shared Function GetExportDate() As DateTime
        Dim strSQL As String = "SELECT ExportDate FROM TimeExport WHERE entryid=1"
        Try
            Return DateTime.Parse(SqlHelper.ExecuteScalar(SqlHelper.SQLConnection, CommandType.Text, strSQL)).[Date]
        Catch
            Return DateTime.Today.AddDays(-15)
        End Try
    End Function

    Public Shared Function GetTotalDailyTimeForUser(selDate As DateTime, employeeID As Long) As DataTable
        Dim dt As DataTable
        Dim ds As DataSet
        Dim endDate As DateTime = selDate.AddMonths(1)
        Dim strSQL As String = "SELECT DAY(timeDate) AS dayofMonth, SUM(timecalc) AS dailySum, SUM(apptimeCalc) AS dailySumApp, COUNT(timeEntryID) AS numEntries, SUM(cbApproved) AS numApproved FROM timeEntry "
        strSQL = strSQL & "WHERE employeeid=" & employeeID & " " & "AND timeDate BETWEEN '" & selDate.ToString("yyyy-MM-dd") & "' AND '" & endDate.ToString("yyyy-MM-dd") & "' " & "GROUP BY timeDate"
        ds = SqlHelper.ExecuteDataset(SqlHelper.SQLConnection, CommandType.Text, strSQL)
        dt = ds.Tables(0)
        Return dt
    End Function

    Public Shared Function GetTimeEntryJobsForThisUserByProjectStatusFilterArray(employeeID As Long, ProjectStatusFilterArray As String()) As DataTable
        Dim strFilterBy As String = ""

        For i As Integer = 0 To ProjectStatusFilterArray.Length - 1
            strFilterBy = (If(strFilterBy = "", "", strFilterBy & " or ")) & "status ='" & ProjectStatusFilterArray(i) & "'"
        Next

        Dim strSQL As String = "SELECT * FROM viewProjectEmployeeDetails WHERE (employeeid=" & employeeID & ")" & " AND (" & strFilterBy & ")" & " ORDER BY projectid"
        Dim ds As DataSet
    ...
    ' Gets the time entry records for the current user and date, including time status information
    Public Shared Function TimeEntryByDate(selDate As DateTime, flds As String, username As String) As DataTable
        Dim endDate As DateTime = selDate.AddDays(1)
        Dim startDate As DateTime = selDate.ToString("yyyy-MM-dd")

        Dim strSQL As String = "SELECT " & flds & " FROM timeentry AS tt " & "LEFT JOIN timeentrystatus AS tts ON tts.timeEntryID=tt.timeEntryID " & "WHERE tt.employeeID='" & username & "' " & "and tt.timeDate >= '" & startDate & "' AND tt.timeDate < '" & endDate & "' " & "ORDER BY tt.timeDate asc, tt.timeEntryID"
        Dim ds As DataSet

        ds = SqlHelper.ExecuteDataset(SqlHelper.SQLConnection, CommandType.Text, strSQL)
        Dim dt As DataTable = ds.Tables(0)

        dt.Columns.Add("StartTimeHr", System.Type.[GetType]("System.String"))
        dt.Columns.Add("StartTimeMin", System.Type.[GetType]("System.String"))
        dt.Columns.Add("EndTimeHr", System.Type.[GetType]("System.String"))
        dt.Columns.Add("EndTimeMin", System.Type.[GetType]("System.String"))
        dt.Columns.Add("UseMinHrTimes", System.Type.[GetType]("System.Boolean"))

        If dt.Rows.Count > 0 Then
            Dim i As Int32 = 0
            While i < dt.Rows.Count
                dt.Rows(i)("StartTimeHr") = Convert.ToDateTime(dt.Rows(i)("start_time")).Hour.ToString()
                dt.Rows(i)("StartTimeMin") = Convert.ToDateTime(dt.Rows(i)("start_time")).Minute.ToString()
                dt.Rows(i)("EndTimeHr") = Convert.ToDateTime(dt.Rows(i)("end_time")).Hour.ToString()
                dt.Rows(i)("EndTimeMin") = Convert.ToDateTime(dt.Rows(i)("end_time")).Minute.ToString()
                dt.Rows(i)("UseMinHrTimes") = False
                i = i + 1
            End While
        End If
    ...
    Public Shared Function GetUserFullName(employeeID As Long) As DataTable
        Dim strSQL As String = "SELECT CONCAT(fname, ' ', lname) AS fullname FROM employees " & "WHERE employeeid=" & employeeID
        Dim ds As DataSet
        ds = SqlHelper.ExecuteDataset(SqlHelper.SQLConnection, CommandType.Text, strSQL)
        Dim dt As DataTable = ds.Tables(0)
        Return dt
...
#Region "Servers"
    ...
    Public Shared ReadOnly Property SQLConnection() As String
        ...
        gdbConnection = System.Configuration.ConfigurationManager.ConnectionStrings("pmwConnectionString").ToString
      End If
